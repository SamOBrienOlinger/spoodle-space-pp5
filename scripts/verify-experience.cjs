...
(async()=>{
  ...
  try{
    for(const width of [320,390,430,768,1024,1440]){
      ...
      assert(metrics.overflow<=1,`Overflow at ${width}`);assert.equal(metrics.nav,'rgb(25, 47, 70)');assert.equal(metrics.gradient,'none');assert(metrics.logo>0);assert.equal(metrics.unsupported,false);assert.equal(metrics.columns,width>=992?2:0);
