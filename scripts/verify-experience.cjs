...
(async()=>{
  ...
  try{
    for(const width of [320,390,430,768,1024,1440]){
      ...
      assert(metrics.overflow<=1,`Overflow at ${width}`);assert.equal(metrics.nav,'rgb(89, 36, 130)');assert(metrics.gradient.includes('linear-gradient'), 'Purple header gradient must render');assert(metrics.logo>0);assert.equal(metrics.unsupported,false);assert.equal(metrics.columns,width>=992?2:0);
    ...
    await page.close();
    const reduced=await browser.newPage({viewport:{width:390,height:844},reducedMotion:'reduce'});await fixtures(reduced);await ready(reduced);assert.equal(await reduced.locator('[style*="--enter-delay"]').first().evaluate(e=>getComputedStyle(e).animationName),'none');report.checks.push('Reduced-motion preference disables entrance animations');await reduced.close();
