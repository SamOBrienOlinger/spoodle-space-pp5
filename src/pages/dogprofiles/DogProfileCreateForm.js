...
function DogProfileCreateForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      NotificationManager.success('Doggy profile created!', 'Success');
      history.push(`/dogprofiles/${data.id}`);
    } catch (err) {
      if (err.response?.status !== 401) {
        setErrors(formErrors(err));
        NotificationManager.error('Please check the form and try again', 'Oopsadoodle!')      }
    }
  };

  return (
    <FormPage title="Doggy profile" description="Introduce your dog to the community." icon="paw" onSubmit={handleSubmit} errors={errors} submitLabel="Create Doggy profile" busyLabel="Saving…" onCancel={() => history.goBack()}>
